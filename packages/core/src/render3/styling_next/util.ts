...
*/
import {SafeValueImpl, unwrapSafeValue} from '../../sanitization/bypass';
import {TNode, TNodeFlags} from '../interfaces/node';

import {LStylingData, StylingConfig, StylingMapArray, StylingMapArrayIndex, TStylingContext, TStylingContextIndex, TStylingContextPropConfigFlags} from './interfaces';
...
export function allocTStylingContext(initialStyling?: StylingMapArray | null): TStylingContext {
  ...
  return [
    StylingConfig.Initial,  // 1) config for the styling context
...
export function hasConfig(context: TStylingContext, flag: StylingConfig, bitsMustMatch?: boolean) {
  const value = getConfig(context) & flag;
  return bitsMustMatch ? value === flag : value > 0;
}

export function allowDirectStylingApply(context: TStylingContext, hostBindingsMode: boolean) {
  return isContextLocked(context, hostBindingsMode) &&
      !hasConfig(context, StylingConfig.HasCollisions) &&
      !hasConfig(context, StylingConfig.HasPropAndMapBindings, true);
}

export function setConfig(context: TStylingContext, value: StylingConfig) {
  context[TStylingContextIndex.ConfigPosition] = value;
}

export function patchConfig(context: TStylingContext, flag: StylingConfig) {
...
export function lockContext(context: TStylingContext, hostBindingsMode: boolean) {
  const flag =
      hostBindingsMode ? StylingConfig.HostBindingsLocked : StylingConfig.TemplateBindingsLocked;
  setConfig(context, getConfig(context) | flag);
}
...
export function isStylingMapArray(value: TStylingContext | StylingMapArray | null): boolean {
  // the StylingMapArray is in the format of [initial, prop, string, prop, string]
  // and this is the defining value to distinguish between arrays
  return Array.isArray(value) && isStylingContext(value) === false;
}
...
export function normalizeStylingValue(value: any) {
  return value instanceof SafeValueImpl ? unwrapSafeValue(value) : value;
}
